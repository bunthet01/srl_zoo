...
class GeneratorResNet(BaseModelAutoEncoder):
    ...
    def __init__(self, state_dim, img_shape, label_dim, spectral_norm=False, device='cpu'):
...
class DiscriminatorDC(nn.Module):
    def __init__(self, state_dim, label_dim, img_shape,spectral_norm=False,device='cpu'):
    ...
    def forward_cgan(self, input, label, target):
...
class CGanNewTrainer(BaseTrainer):
    ...
    def build_model(self, model_type='dc'):
        ...
        if model_type == "dc":
            self.generator = GeneratorDC(self.state_dim, self.label_dim, self.img_shape, spectral_norm=True,device=self.device)
            self.discriminator = DiscriminatorDC(self.state_dim, self.label_dim, self.img_shape, spectral_norm=True,device=self.device)
    ...
    def train_on_batch(self, obs, label ,target_pos, epoch_batches,figdir,epoch,fixed_sample_state,
        ...
        output = self.discriminator.forward_cgan(obs, label, target_pos)
        ...
        fake_obs = self.generator.forward_cgan(sample_state, sample_label, sample_t_pos)
        if add_noise:
        ...
        output = self.discriminator.forward_cgan(fake_obs.detach(), sample_label, sample_t_pos)
        ...
        output = self.discriminator.forward_cgan(fake_obs, sample_label, sample_t_pos)
        ...
        if epoch_batches % (minibatch-1) == 0 and not valid_mode and figdir is not None :
            fake_obs = self.generator.forward_cgan(fixed_sample_state.to(device), fixed_sample_label, fixed_sample_target_pos)
